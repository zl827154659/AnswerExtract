...
#
def evaluate_cmrc(ground_truth_file, prediction_file):
    f1 = 0
    em = 0
    total_count = 0
    skip_count = 0
    for instance in ground_truth_file:
        ...
        f1 += calc_f1_score(answer_text, prediction)
        em += calc_em_score(answer_text, prediction)
...
def calc_f1_score(answer_text, prediction):
    f1_scores = []
    ans_segs = mixed_segmentation(answer_text, rm_punc=True)
...
def calc_em_score(answer_text, prediction):
    em = 0
    ans_ = remove_punctuation(answer_text)
...
if __name__ == '__main__':
    ...
    print(f'start_eval')
    F1, EM, TOTAL, SKIP = evaluate_cmrc(ground_truth_file, prediction_file)
    AVG = (EM + F1) * 0.5
